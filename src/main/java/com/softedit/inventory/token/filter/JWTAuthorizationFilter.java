...
import com.softedit.inventory.token.JWTTokenProvider;
...
import static com.softedit.inventory.constant.SecurityConstant.TOKEN_PREFIX;
...
public class JWTAuthorizationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (request.getMethod().equalsIgnoreCase(SecurityConstant.OPTIONS_HTTP_METHOD)){
            ...
            // The options is seen before every request and is sent to gather information about the server
            ...
            // If the header does not contain "Bearer " it means that this is not the authorization header that we are looking for
            ...
            if(authorizationHeader == null || !authorizationHeader.startsWith(TOKEN_PREFIX)){
                ...
                // When we do the method above, we just let the request continue it's flow and then we stop the method with return
                return;
            }
